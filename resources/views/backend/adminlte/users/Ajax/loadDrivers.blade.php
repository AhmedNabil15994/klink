...
<table class="table table-hover daTatable dataTable deleteFormModal text-center demo-foo-filtering10" data-form="deleteForm"  >
            <thead>
                    ...
                    <tr class="tab-row{{$users['id']}} selected_record">
                        ...
                        <td>@if(!empty($users['email']))
                            ...
                            <div class="row">
                                ...
                                <span class="text-left">
                                    ...
                                    @elseif(!empty($users['postal_code']))
                                    <i class="fa fa-home"></i> {{$users['postal_code']}}
                                    @endif
                                </span>
                                <span class="text-left">
                                    @if(!empty($users->profile->phone))
                                    <i class="fa fa-phone"></i> {{$users->profile->phone}}
                                    @elseif(!empty($users['phone']))
                        ...
                        <td>
                            ...
                            <?php 
                                ...
                                $driver = \DB::table('drivers')->where('profile_id','=',$users['profile_id'])->first();
                            ?>


                            @if(!empty($users['role']))
                            <label class="label">{{$users['role']}}</label>
                            @endif
                            @if(!empty($driver))
                            <label class="label">{{trans('backend/user.company_emp')}}</label>
                            @endif

                        </td>
                        <td>
                            <button type="button" name="delete" class="btn btn-danger btn-xs  delete" alt=" {{trans('backend/user.delete')}}" value="{{$users['id']}}" data-profile="{{$users['profile_id']}}"><i class="fa fa-trash"></i> {{ trans('backend/user.delete') }}</button>                
                            {!! Form::close() !!}
                        </td>
                    </tr>
                </form>
...
<script type="text/javascript">
    $(function(){
        var oTable = $('.demo-foo-filtering10').DataTable();
