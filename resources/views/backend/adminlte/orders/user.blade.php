...
@section('main-content')
    ...
    @include('backend.adminlte.orders.showCompanyOrders')
<style>
    .mybox-title{
    ...
    <div id="home" class="tab-pane active in">
        <div class="pag">
            ...
            <div class="mytable-header">
                ...
                <div class="mytable-cell">{{trans('backend/orders.showUser')}}</div>
            </div>
            <div class="orders">
    ...
    window.onload = function(){
        ...
        function showOrders(pageNum){
            ...
            orders.forEach(element => {
                ...
                $('.orders').append(getOrder(element,i))
                i++;
            });
            
            $('.showCompanyOrders').click(function(){
                var index = $(this).val();
                var order = window.orders[1][index];
                $('#companyModal .modal-title').html(`${order.name}{{trans('backend/orders.userOrders')}}`);
                $('#companyModal #OrderCarsouel .carousel-inner').html(getOrdersTable(order.orders,order.name));
                $('#companyModal').modal('toggle');
                $('[data-toggle="tooltip"]').tooltip(); 
                $('.openOrderModal a').click(function(){
                    console.log($(this).data('order'))
                    
                    ShowModal($(this).data('order'));
                })
            })
        ...
        function ShowModal(order){
            ...
            $('#ShowOrderModal').css('z-index',$('#companyModal').css('z-index')+11);
...
<div class="box collapsed-box" style="border-width:1px; margin-bottom:0;">
    <div class="box-header with-border">
        <h3 class="box-title mybox-title">
            ...
            <div class="mybox-cell">
                <button type="button" class="btn btn-sm btn-success showCompanyOrders" value="${i}">
                    <i class="fa fa-eye"></i>{{trans('backend/orders.showUser')}}
                </button>
            </div>
    ...
    <!-- box-footer -->
        ...
        function getOrdersTable(orders,userName){
            ...
            orders.forEach(function(order){
               ...
               var otherOrder = JSON.parse(JSON.stringify(order));
               otherOrder['offers'] = '';
                ...
                text += `
                        ...
                        <div class="box-header with-border">
                                <h3 class="box-title mybox-title">
                                        ...
                                        <div class="mybox-cell openOrderModal" style="cursor:pointer;"  data-toggle="tooltip" title="{{trans('backend/orders.showMessage')}}"><a data-order='${JSON.stringify(otherOrder)}' href="#">${order.id}</a></div>
